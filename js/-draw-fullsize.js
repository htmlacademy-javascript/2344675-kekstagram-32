...
const closeModal = () => {
  ...
  // fullCardBtnCancel.removeEventListener('click', onCloseModalBtn);
};
...
const drawfullCard = (evt) => {
  const card = evt.target.closest('.picture');
  console.log(card)
  if (card) {
    const thisId = Number(card.dataset.id);
    const photoData = mockPosts
    // (fullCard.querySelector('.big-picture__img img')).src = mockPosts[thisId - 1].url;
    // (fullCard.querySelector('.social__caption')).textContent = mockPosts[thisId - 1].description;
    // (fullCard.querySelector('.likes-count')).textContent = mockPosts[thisId - 1].likes;
    // (fullCard.querySelector('.social__comment-total-count')).textContent = mockPosts[thisId - 1].comments.length;
    ...
    fullCardBtnCancel.addEventListener('click', onCloseModalBtn, { once: true });
  }
};
posts.addEventListener('click', drawfullCard);
